    ...
    while True:
        if joy:
            ...
            if rover_data["mode"] == "DRIVE":
                ...
                # 2. Independent Side Controls from Sticks (SWAPPED!)
                # Reading Axis 3 (Right Stick) for the 'ly' math, and Axis 1 (Left Stick) for the 'ry' math
                raw_ly = -joy.get_axis(3)
                raw_ry = -joy.get_axis(1)
                ...
                # 5. Apply Power Limit AND Global Hardware Polarity
                # Left motors need NEGATIVE to go forward, Right needs POSITIVE.
                rover_data["cmd_l"] = -bl * power_limit
                rover_data["cmd_r"] = br * power_limit
                ...
                # NEW: NEO 7 mapping (Bumpers: RB is forward, LB is reverse at 20% speed)
                if joy.get_button(5): rover_data["neo7"] = 0.2
                elif joy.get_button(4): rover_data["neo7"] = -0.2
                else: rover_data["neo7"] = 0.0
                
                # NEW: NEO 8 mapping (Face Buttons: Y is forward, A is reverse at 20% speed)
