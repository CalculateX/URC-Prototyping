    ...
    while True:
        if joy:
            ...
            if rover_data["mode"] == "DRIVE":
                ...
                # NOTE: I added a minus sign (-) to invert Forward/Backward.
                # NOTE: I swapped `br` to the left side and `bl` to the right side to invert Left/Right.
                ...
                # Servo mapping via controller axes
                analog = 40.0
        ...
        if ser and ser.is_open:
            ...
            msg = f"<{m_char},{rover_data['cmd_l']:.2f},{rover_data['cmd_r']:.2f},{int(rover_data['s1'])},{int(rover_data['s2'])},{int(rover_data['s3'])},{int(rover_data['s4'])},{current_led_char},{rover_data['sci_cmd']}>\n"
